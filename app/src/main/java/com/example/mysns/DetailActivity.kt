        ...
        FirebaseDatabase.getInstance().getReference("/Comments/$postId").addChildEventListener(object
            ...
            override fun onChildMoved(snapshot: DataSnapshot, prevChildKey: String?) {
                if(snapshot != null){
                    ...
                    comment?.let{
                        val existIndex = commentList.map { it.commnetId }.indexOf(it.commnetId)
                        commentList.removeAt(existIndex)

                        val prevIndex = commentList.map { it.commnetId }.indexOf(prevChildKey)
            ...
            override fun onChildRemoved(snapshot: DataSnapshot) {
                snapshot?.let {
                    val comment= snapshot.getValue(Comment::class.java)
                    comment?.let{ comment ->
                        val existIndex = commentList.map { it.commnetId }.indexOf(comment.commnetId)
                        commentList.removeAt(existIndex)
